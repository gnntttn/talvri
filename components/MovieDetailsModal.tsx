...
const MiniMovieCard: React.FC<{ movie: Movie; onSelect: (movie: Movie) => void }> = ({ movie, onSelect }) => {
  ...
  return (
    <div className="flex-shrink-0 w-32 cursor-pointer group" onClick={() => onSelect(movie)}>
      ...
      <h4 className="text-sm font-semibold mt-2 truncate">{movie.title}</h4>
    </div>
  )
};
...
export const MovieDetailsModal: React.FC<MovieDetailsModalProps> = ({ movie, onClose, isFavorite, onToggleFavorite, isWatchlisted, onToggleWatchlist, onSelectSimilarMovie, onSelectPerson, playOnMount = false }) => {
  ...
  useEffect(() => {
    ...
    getSimilarMovies(movie.id, 1, language).then(data => setSimilarMovies(data.results.slice(0, 10))).catch(err => console.error("Failed to fetch similar movies", err));

    return () => { window.removeEventListener('keydown', handleEsc); document.body.style.overflow = 'auto'; };
  }, [onClose, movie.id, language, playOnMount, showTrailer]);

  const renderTabContent = () => {
    switch(activeTab) {
        case 'overview': return (<div><h3 className="text-xl font-bold mb-2">{t('overview')}</h3><p className="text-slate-600 dark:text-slate-300 text-sm">{movie.overview}</p></div>);
        case 'cast': return ((topCredits.crew.length > 0 || topCredits.cast.length > 0) && (<div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1">{topCredits.crew.map(m => <CreditListItem key={`crew-${m.id}`} person={m} subtitle={m.job} onSelect={onSelectPerson} />)}{topCredits.cast.map(m => <CreditListItem key={`cast-${m.id}`} person={m} subtitle={m.character} onSelect={onSelectPerson} />)}</div>));
            ...
            <div className="px-6 md:px-8 pb-8">
                <div className="flex flex-col sm:flex-row -mt-24 sm:-mt-28 relative items-end gap-6">
                    <div className="w-36 sm:w-48 flex-shrink-0">
                        {posterUrl && <img src={posterUrl} alt={movie.title} className="w-full rounded-lg shadow-2xl object-cover" />}
                    </div>
                    <div className="flex-grow pb-2">
                        <h2 className="text-3xl font-black">{movie.title}</h2>
                        {movie.tagline && <p className="hidden sm:block text-md text-slate-500 dark:text-slate-400 mt-1">{movie.tagline}</p>}
                    </div>
                </div>
                ...
                <div className="flex flex-col lg:flex-row justify-between items-center gap-4 mt-6">
                    <div className="flex flex-wrap items-center justify-center lg:justify-start gap-x-3 sm:gap-x-4 gap-y-2">
                        ...
                        <span className="text-md font-semibold">{releaseYear}</span>
                        <span className="text-slate-500 hidden sm:inline-block">•</span>
                        <span className="text-md font-semibold">{runtime}</span>
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                        ...
                        <button onClick={() => onToggleWatchlist(movie)} className="p-2.5 rounded-full bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors" aria-label={isWatchlisted ? t('removeFromWatchlist', { title: movie.title }) : t('addToWatchlist', { title: movie.title })}><BookmarkIcon className={`w-5 h-5 ${isWatchlisted ? 'text-indigo-500' : 'text-slate-500 dark:text-slate-400'}`} fill={isWatchlisted ? 'currentColor' : 'none'} stroke="currentColor" /></button>
                        <button onClick={() => onToggleFavorite(movie)} className="p-2.5 rounded-full bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors" aria-label={isFavorite ? t('removeFromFavorites', { title: movie.title }) : t('addToFavorites', { title: movie.title })}><HeartIcon className={`w-5 h-5 ${isFavorite ? 'text-indigo-500' : 'text-slate-500 dark:text-slate-400'}`} fill={isFavorite ? 'currentColor' : 'none'} stroke="currentColor" /></button>
                    </div>
                </div>

                <div className="mt-8 border-b border-slate-200 dark:border-slate-700">
                    <div className="flex space-x-2 rtl:space-x-reverse -mb-px overflow-x-auto"><TabButton label={t('overview')} isActive={activeTab === 'overview'} onClick={() => setActiveTab('overview')} /><TabButton label={t('castAndCrew')} isActive={activeTab === 'cast'} onClick={() => setActiveTab('cast')} /><TabButton label={t('whereToWatch')} isActive={activeTab === 'watch'} onClick={() => setActiveTab('watch')} /><TabButton label={t('reviews')} isActive={activeTab === 'reviews'} onClick={() => setActiveTab('reviews')} /></div>
                </div>

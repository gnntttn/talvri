...
const SideMenuItem: React.FC<{
  ...
  const inactiveClasses = 'text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800';
...
export const SideMenu: React.FC<SideMenuProps> = ({ isOpen, onClose, activeTab, setActiveTab }) => {
  ...
  return (
    <>
      ...
      <div 
        className={`fixed top-0 left-0 bottom-0 w-4/5 max-w-xs bg-slate-100 dark:bg-slate-900 z-50 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} rtl:left-auto rtl:right-0 ${isOpen ? 'rtl:translate-x-0' : 'rtl:translate-x-full'}`}
        role="dialog"
        aria-modal="true"
      >
